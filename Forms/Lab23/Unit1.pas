...
Begin
    ...
    Begin
        IsCorrect := True;
        Col := 0;
        While (IsCorrect) And (Col <= High(Matrix)) Do
        Begin
            If (Matrix[Row][Col] <> Col + 1) Then
                IsCorrect := False;
            Inc(Col);
        End;
        If IsCorrect Then
...
Begin
    ReWrite(F);
    Write(F, 'Количество строк матрицы A порядка n, элементы которых представляют перестановки чисел от 1 до n: ', ResultEdit.Text);
    CloseFile(F);
End;
